...
d_artist() {
    mpc list artist | sort -f | dmenu -l 10 -g 5 -h 50 -p Artist "${dmenu_args[@]}"
}
    ...
    if (( ${#albums[@]} > 1 )) ; then
        {
            printf '%s\n' "$all_name"
            printf '%s\n' "${albums[@]}" | sort -f
        } | dmenu -l "$((${#albums[@]} + 1))" -h 50 -p Album "${dmenu_args[@]}"
    else
...
d_playlist() {
    ...
    track=$(mpc playlist -f "$format" | dmenu -c -l 10 -h 50 -p Track "${dmenu_args[@]}")
    printf '%s' "${track%% *}"
}
